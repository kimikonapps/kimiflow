...
def worktree_status(path):
    ...
    proc = run_git(path, ["status", "--porcelain=v2", "-z", "--branch", "--untracked-files=all"], check=False)
    if proc.returncode != 0:
        raise WorkspaceError("cannot inspect worktree status: %s" % path)
    result = parse_status_v2(proc.stdout)
    ignored = run_git(path, ["ls-files", "--others", "--ignored", "--exclude-standard", "-z"], check=False)
    if ignored.returncode != 0:
        raise WorkspaceError("cannot inspect ignored worktree content: %s" % path)
    ignored_paths = [item.decode("utf-8", "surrogateescape") for item in ignored.stdout.split(b"\0") if item]
    result["ignored_paths"] = ignored_paths[:IGNORED_PATH_SAMPLE_LIMIT]
    result["ignored_count"] = len(ignored_paths)
    result["ignored_paths_truncated"] = len(ignored_paths) > IGNORED_PATH_SAMPLE_LIMIT
    return result

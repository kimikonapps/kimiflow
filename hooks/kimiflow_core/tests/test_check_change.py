...
from kimiflow_core import check_change as checks
...
class CheckChangeTests(unittest.TestCase):
    ...
    def test_installed_helper_cannot_be_shadowed_by_project_package(self):
        package = self.root / "kimiflow_core"
        package.mkdir()
        (package / "__init__.py").write_text("raise RuntimeError('wrong package')")
        helper = Path(__file__).resolve().parents[2] / "check-change.sh"
        result = subprocess.run(
            [str(helper), "--root", str(self.root), "--check", json.dumps(self.py("pass"))],

...
run_static() {
  ...
  assert_contains "$skill" "treat product intake and confirmation as complete"
  assert_contains "$skill" "FirstMate alone owns"
  assert_contains "$skill" "Standalone behavior"
  for action in activate spawn status send drain teardown; do
...
for line_no, line in enumerate(open(sys.argv[1], encoding="utf-8"), 1):
    ...
    for item in message.get("content") or []:
        ...
        if message.get("role") == "assistant" and item.get("type") == "text":
            if item.get("text", "").strip().rstrip(".") == "KIMIFLOW_PROJECT_MAIN_HANDLED":
...
for required in ("activate", "spawn", "drain", "status"):
...
run_live() {
  ...
  prompt="Beginne diesen bestaetigten Kimiflow-Live-Run in dieser Main-Sitzung. Verwende ausschliesslich das Tool kimiflow_crew: aktiviere die Crew und starte den sichtbaren Ship $task mit folgendem bereits bestaetigten Brief. Brief: Erstelle nur kimiflow-live-proof.txt mit exakt stock-firstmate-kimiflow-ok, verifiziere Inhalt und git diff --check, committe lokal und melde done ueber den FirstMate-Status. Wiederhole kein Produkt-Intake. Nach worker_reachable: nicht pollen, keine Fortschrittsmeldungen, sondern den Turn ruhig beenden und auf den automatischen FirstMate-Wake warten. Nach genau diesem Wake: einmal drain, einmal status und antworte erst bei done exakt KIMIFLOW_PROJECT_MAIN_HANDLED. Verwende keine direkten FirstMate- oder Herdr-Shellbefehle."

...
Conditional Reference Strategy Fit reuses the same Phase-2 depth. It starts from a plan-changing question after local understanding (and, for fixes, after reproduction and root-cause proof), compares at most two pulse or three focused references total across the run's fit assessment, and records compact mechanism/invariant/trade-off/fit cards. Pulse may promote once; focused exhaustion gets one local falsification and, for risky/repeated failure, one top-model recovery pass before the top model selects the smallest reversible supported strategy. A low Current State freshness risk does not suppress a named strategy gap, and a research limit or source conflict never creates a user approval stop.

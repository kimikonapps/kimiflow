...
| `write_global_economics_row` row shape | pinned Bash global row carries no `scope` field | port appends `"scope"` after `run_type` (from STATE.md `Scope:` line via `run_scope_from_state`; `trivial\|small\|large\|unknown`) | All 40 recorded rows were scope-blind, so the open recall-value-per-scope question was unanswerable from the ledger. Additive enum inside the existing privacy allowlist; readers use `.get`. The parity comparisons (test_economics, test_review `__global__`) strip the field before the byte diff. User-blessed 2026-07-05. |
...
| `cmd_propose` unknown-id gate (3851-3854) | the gate `[$ids[] \| select(($known \| index(.)) == null)]` is **dead code**: inside `$known \| index(.)` the `.` rebinds to `$known` itself, so it is a subarray-self-search that returns `0` (never `null`); `missing` is always `[]` and the `die "propose: unknown proposal id(s)" 2` never fires. An unknown `--approve`/`--reject` id is silently accepted (written, but no proposal matches it). | the port **omits** the gate entirely (the approve/reject MATCHING `id in approve_ids` already no-ops for an unknown id) | **Replicated bug, not a divergence.** The gate was clearly intended to reject typo'd ids, but the jq `.`-rebinding makes it inert; a real exit-2 check would diverge from the Bash that ships. Caught by byte-for-byte grounding (`--approve <unknown>` exits 0/written on both). Same drop-in-fidelity call as the `resolve_run_dir` quirk. |
| `write_global_economics_row` write-phase failure reasons (3099-3115) | distinguishes `mkdir_failed` / `mktemp_failed` / `write_failed` / `move_failed` from the separate `mkdir`/`mktemp`/`jq`/`mv` steps | the port emits `mkdir_failed` (os.makedirs) and `write_failed` (any `store.atomic_write` failure - mkstemp/write/replace collapse into one surface) | The granular `mktemp_failed`/`move_failed` reasons come from the stepwise Bash IO that the stdlib atomic writer collapses; all are unreachable on supported hosts (the metrics dir is writable). The **reachable** outcomes - `disabled` / `home_unavailable` / `salt_unavailable` / success, plus the project + global ledger bytes - are byte-for-byte identical (grounded incl. the dedupe/existing-file branch). The project ledger gets `mode=0o600` to match `mktemp`(0600)+`mv`. |
| `sanitize_evidence_ref` root check (`case "$root"/*\|"$root"`) — **audit fix B3-P1 (2026-07-02)** | raw string-prefix check: a `../`-ref (`../../etc/hosts`, `/root/../etc/passwd`) passes as in-repo, so `evidence_fingerprints_json` hashes and fingerprints out-of-repo files (e.g. `/etc/hosts` as `status:"current"`) | the joined path AND the root are `os.path.normpath`-normalized (lexical only, no symlink resolution) before the prefix check; escaping refs map to `OUTSIDE_REPO`/`outside_root`, in-repo refs with redundant segments (`src/../src/foo.py`, `src//foo.py`) are stored normalized | Path-traversal fix: evidence refs must never fingerprint files outside the repo root. normpath (not realpath) keeps symlinked roots (e.g. `/tmp` on macOS) matching their own prefix. Clean in-repo refs are byte-identical; the record parity cases use clean refs, so no harness diff. |
| `append_learning_row` full-rewrite (2405-2512, `jq ... > tmp; mv`) — **audit fix B3-P2 (2026-07-02)** | the rewrite path (status=current + existing rows file) re-serializes only the leniently parsed rows: blank/malformed/non-object lines are silently dropped, while the append path keeps them — inconsistent, and a data-loss surface for hand-edited files | `store.read_jsonl_with_lines` pairs every raw line with its parsed dict (or `None`); the rewrite re-serializes parsed rows in place and keeps all other lines verbatim. Non-dict JSON lines (e.g. a bare `5`) count as unparsed — they are preserved verbatim AND excluded from dedup/supersession, where the old `read_jsonl`-based code would have crashed on `row.get` | Data-preservation fix: a rewrite triggered by one new learning must not destroy unrelated bytes. Parsed-row serialization is unchanged (jq-compact), so well-formed files are byte-identical. |
| `append_learning_row` security-gate scope (2415-2420) — **audit fix B3-P3 (2026-07-02)** | `memory_security_json` scans only `summary`; injection phrases or hidden unicode in `topic`/`evidence` pass the gate, and bare secret VALUES (the phrase patterns match phrases, not values) keep `sensitivity=normal` — such rows enter the VAULT-SYNC candidate set | the gate scans the newline-joined `summary`+`topic`+`evidence` (phrase windows use `.`, which cannot cross the `\n` field boundaries — no new cross-field matches); a minimal secret-VALUE pattern class (`rows.has_secret_value`: AWS access-key ids, PEM private-key headers, GitHub/Slack tokens, `key=value` assignments with a ≥16-char literal) forces `sensitivity=security` without closing the gate | Security fix: the gate's stored `security_scan` now covers every free-text field of the row, and secret values are quarantined from vault sync (provider `_sync_base_candidates` drops `security` rows) while the learning itself is still recorded. Benign rows are byte-identical (`ok:true, reasons:[]`), so the record parity cases do not diff. |

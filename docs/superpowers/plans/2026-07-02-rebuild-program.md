...
- The invariants artifact (`2026-07-02-token-restructuring-invariants.md`) and needle-check gate every commit that touches SKILL.md / reference.md / rendered ports. When a rule moves (SKILL → gate, SKILL → phase file, SKILL → rendered source), the needle moves **in the same commit** and the check must pass before and after.
...
3. **Extend the invariant gate before moving prose:** convert `docs/superpowers/plans/2026-07-02-invariant-check.sh` from SKILL/reference-only greps into a target-map check whose rows can point at SKILL.md, reference.md, `phases/*.md`, hook code, hook tests, and rendered host files. The extended check must pass against the pre-move state before any rule relocation commit.
4. **Implement per approved group:**
   - `GATE-CHECKABLE` → gate/check in `kimiflow_core` (test-first), THEN the prose copy shrinks to a one-line pointer ("enforced by <hook>"). The needle relocates to the gate's test.
...
- Per commit: full suite loop (all `hooks/test-*.sh` except the 2 production hooks — this includes the Python unit wrappers) + both smokes + `release-consistency-check.sh` + needle-check on any SKILL/reference/phase-file edit + CHANGELOG Unreleased entry.

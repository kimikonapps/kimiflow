...
- **Phase 2 protected rules:** top owns Discovery/synthesis/triage and Reference Strategy Fit. Depth follows decision need, not size; normally no worker, at most two independent lanes. A prior-work cue forces one bounded local recall; strategy recall adds at most one verified success and failure. Fix proves Red/cause first. `required|default|optional` protects scope; only `required` may add it. Technical gaps change evidence or strategy; only irreducible product/policy choices ask the user.
...
- **Phase 7 protected rules:** frontend preflight must be OPEN and Contract-1 `clean/no`. Review from the immutable run start across committed, staged, unstaged, and named untracked files; pin each round's target. Add R3 for contracts/multiple surfaces; count only verified candidates and refute HIGH. Triage advisories. Schema 4 commits named paths with `git commit --only`; foreign staged work stays untouched and push/release stay explicit. Learning CLOSED blocks completion; finish then records the evidence-gated outcome and retires safely.

...
- **Phase 4 held rule:** only evidenced BLOCKER/HIGH revises; valid findings and gate-local round numbers never reset. Per-strategy budget: small 2, large/audit 3; oscillation/cap starts autonomous recovery, never a continue prompt. Schema-3 fix asks once after diagnosis/plan review, records a basis-bound approval with `clarify-gate.sh --record-fix-approval`, requires `--post-diagnosis`, and skips generic Build Preview; any basis change closes approval.

...
- **Natural mode aliases:** **`/kimiflow full|grill|plan|build|quick|review|audit|fix [target]`** and plain text such as **`kimiflow full`** are first-class shortcuts. If the target is omitted, use the current conversation topic only when it is unambiguous; otherwise ask one plain-language question. Alias meanings:
  - **`full`** — scope=`large` strict loop; after the plan-gate, **STOP for one mode-specific Preview approval** (Build for feature/audit, Fix after diagnosis), never `PLAN.md` approval.
  ...
  - **`quick`** — lean small/low-risk: feature confirms intent with no question minimum; fix defers approval until diagnosis; no-worker Discovery default; **skips Phase-2 recall/Vault Pulse**; one review lens. Never for `full|grill|plan`.
  ...
  - **`fix`** — alias for `--fix`: problem brief → Red/cause/research → one post-diagnosis Fix Preview → Green/regression.
- **`/kimiflow … --prepare`** — prepare only: phases 0–4, then STOP. Package in `.kimiflow/<slug>/`; implement later, even in a new session.
- **`/kimiflow --resume <slug>`** — read STATE, run resume safety, revalidate changed plans, and regenerate the mode-specific Preview before Phase 5. Unknown basis/paths forbid blind build. No slug → list runs and ask.
...
- **Build/Fix Preview:** feature/audit uses conditional risk approval; fix uses one always-confirmed post-diagnosis Preview, never a second pre-build prompt. `full` waits once.
...
- **Model routing.** `top` owns orchestration/planning/verdicts/risky diagnosis; `balanced` may implement bounded work; `cheap` only deterministic collection/support. Never let lower tiers orchestrate. Fallback → reference.md "Model routing (per-role)".
...
- **Stop criteria:** success ends; review cap/oscillation changes strategy and continues; recovery awaits only missing authority/external access. Existing Preview/Commit gates remain. Never bypass a gate or repeat a failed strategy.
...
|---|---|---|
| 0 Setup, Routing & Scope-Gate | `phases/phase-0-setup.md` | model/session; `working-tree-gate.sh`; frontend Contract-1 start receipt; scope/verbosity. |
| 1 Clarify | `phases/phase-1-clarify.md` | `clarify-gate.sh`; feature/audit confirmation; fix problem brief without an early stop; no question minimum. |
...
| 4 Plan-gate / approval | `phases/phase-4-review-approval.md` | plan/review resolvers; mode-specific Build/Fix Preview; risk-policy CONTINUE/STOP/PARK. |
...
- **Phase 4 held rule:** only evidenced BLOCKER/HIGH revises; valid findings and round numbers never reset. Budgets: small 2, large/audit 3. Oscillation/cap recovers autonomously; later epochs require matching STATE/RECOVERY receipts. Schema-3 fix asks once, records an authority-bound approval with `clarify-gate.sh --record-fix-approval`, requires `--post-diagnosis`, and skips generic Build Preview; technical recovery preserves it, while changed problem/acceptance/scope/risk closes it.
- **Phase 5 protected rules:** the Red test commit is tests-only and production code never rides along. Deletions require proof; no proof → don't delete. Unclear/repeated technical failure changes hypothesis/approach; only missing authority asks the user.
...
- **Phase 7 protected rules:** preflight requires the frontend gate OPEN and Contract-1 `clean/no` (legacy/read-only only `not-required`) before code recovery. Add reviewer R3 for hooks/plugins/contracts/multiple surfaces; count only verified promoted `CANDIDATE`s and actively refute HIGH. Run advisories; Commit waits for explicit OK and stages named paths only. Learning CLOSED blocks completion; use explicit `--skip` when none. Audit: one slice/commit. Only then `Status: done`.

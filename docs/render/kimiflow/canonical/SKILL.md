...
- **Adaptive control.** Classify scope/intent/domain/operations locally; never infer missing product choices. Rollover only under measured pressure, lower model tiers only after clean evals, Vault context only through its namespace, and old runs only through verified retention. Missing support falls back to the existing flow without a user gate. → reference.md "Adaptive control plane".
...
- **Phase 3 protected rules:** one flat minimum-complete, subtracted, AC-mapped plan. Contract 1 adds 1–8 checkable slices; critical classes bind invariant, AC, typed falsifier, reset. Active conditional contracts map to existing/at most one new AC. Dual-plan adoption takes isolated elements only; blockers never reach review.
...
- **Phase 7 protected rules:** preflights OPEN. Review immutable-start delta; verify candidates/refute HIGH. Persist scoped verified principles and actual model outcomes; archive at most one old terminal run safely. Commit named paths, preserve foreign staging; push/release explicit. Learning CLOSED blocks; finish is transactional.

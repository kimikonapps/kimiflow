...
- **Phase 2 protected rules:** top owns Discovery/synthesis/triage and conditional Reference Strategy Fit. `none|pulse|focused` follows decision need, not size; no worker by default, one focused worker normally, at most two independent lanes. An explicit prior-work cue forces one bounded local recall at any scope. Fix proves Red/cause before adaptive fit research. `required|default|optional` protects scope; only `required` may add it. Technical gaps/limits change evidence or strategy and continue; only irreducible product/policy choices ask the user.

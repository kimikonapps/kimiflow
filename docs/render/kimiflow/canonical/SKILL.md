...
- **Simplicity-first.** Minimal code/plan for the problem. No speculative abstractions, no features beyond the request. Before new code, climb the ladder: does this need to exist at all? → stdlib before hand-rolled → native platform features before a new dependency → one line before fifty.

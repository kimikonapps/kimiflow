...
- **Workspace + Active Session Contract.** `workspace-preflight.sh`: free primary direct; busy/dirty gets one locked `codex/<slug>*`; then FIFO. Foreign/Codex untouched. Phase 3 binds paths/contracts to PLAN; Phase 5 gates writes. Commit → merge-tree/argv integration → finish → ancestry retire. `working-tree-gate.sh` uses its root.
...
- **Phase 2 protected rules:** top owns Discovery/synthesis/triage/fit; standards and Vault recall are scope/path bounded. Architecture/domain/operations activate only from named evidence and require matching plan/verify checks. Technical gaps change strategy; only irreducible product/policy choices ask.
...
- **Phase 7 protected rules:** preflights OPEN. Review start delta; verify candidates/refute HIGH. Persist scoped principles/outcomes; archive at most one old terminal run. Commit named paths, preserve foreign staging. Managed trees integrate from primary, then finish/retire; failures stay recoverable. Push/release explicit. Learning CLOSED blocks; finish is transactional.

## Scaling Knobs

Details: `docs/kimiflow-scaling-knobs.md`. Display verbosity changes output only, never gates/cost/quality. Solo-dev uses one path per run and at most one managed tree; behavioral evals never run in CI.

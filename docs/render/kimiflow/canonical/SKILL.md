...
- **Phase 4 held rule:** only evidenced BLOCKER/HIGH revises; rounds never reset. Small feature/fix cap 2, large/audit cap 3; oscillation/cap stops CLOSED. Schema-3 fix asks once after diagnosis/plan review, records a basis-bound approval with `clarify-gate.sh --record-fix-approval`, requires `--post-diagnosis`, and skips generic Build Preview; any basis change closes approval.

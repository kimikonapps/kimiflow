...
- Kimiflow's clarify gate uses `$KIMIFLOW_PLUGIN_ROOT/hooks/clarify-gate.sh`. New Contract-4 non-trivial features require one explicit bounded Product Intake before planning or writes and must confirm the concrete entry, user interaction, visible delegation outcome, unchanged path, and done scenario. Technical HOW stays agent-owned. At most one causal second batch is allowed when the first answer creates a new material product conflict. Content-free receipts and an Active-Run-pinned intent lock preserve privacy and scope; existing Contract-3 runs, audits, fixes, trivial work, and schema 3 retain compatible contracts.

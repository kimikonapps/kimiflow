...
### Changed
- **Autonomous recovery is mechanically bound to authority and strategy state** (`clarify-gate.sh`, `active-run.sh`, `resolve-review-gate.sh`): schema-3 Fix Preview approval now protects problem/acceptance/scope/risk while technical diagnosis and plan recovery remain autonomous; typed `await-user --kind` rejects Preview/Commit pauses during active recovery; and every later strategy epoch requires a matching STATE/RECOVERY receipt with contiguous rounds and changed fingerprints.

...
Read installed `references/legacy-workflow.md` once. Per phase call `active-run.sh phase-read --run .kimiflow/<slug> --phase <N> --file phases/<phase-file> --packet --write`; trust its packet and do not read phase/reference files separately. Never preload all `reference.md`. Then apply this host map.
...
Apply the canonical Kimiflow workflow from `$KIMIFLOW_PLUGIN_ROOT/references/legacy-workflow.md` with these Codex substitutions:
...
- Large `full|plan`: once material unknowns are resolved, one Sol `fork_turns="none"` worker replaces parent planning through Phase 3 and returns `READY_FOR_CONFIRMATION|NEEDS_DECISION`. The parent asks the one Phase-4 product-contract confirmation, then completes the frozen-plan review; it never repeats that confirmation for a clean risk preview.
...
- Active Session and Adaptive Execution contracts use `active-run.sh`; the origin task owns the run. Fresh non-trivial Contract-4 features defer `start` until the final Phase-4 question. Codex arms intake first; the next direct owner prompt/native response writes its receipt. `hook-health` is diagnostic only. Stop coalesces same-turn observation; `status`/`next-action` read and `observe` records evidence. Exact `abort|park|fail --reason ... --write` stays available during intake. Pending rollover uses one fresh `top` worker with `fork_turns="none"`; pass literal roots, commands, receipts, phase and named code, persist, then call `rollover-handoff`. Shared-checkout edits require `conflict-check`=`allow_disjoint`; use canonical helpers.
- `$KIMIFLOW_PLUGIN_ROOT/hooks/workspace-preflight.sh` routes: free Primary direct; busy/dirty gets ≤3 locked owned Fleet trees, then FIFO. Work in its returned root. Phase 3 `declare`s exclusive Primary/Fleet path-contract leases + `blocked_by` against PLAN; Phase 5 requires `write-gate` and post-advance `revalidate`. Commit, `integrate` the JSON-argv-checked combined candidate, then `needs-reconcile` or finish/`retire`. External Fleet adoption uses `recover-integrated`; never edit receipts. Never mutate manual/Codex trees or request routine commit/stash/clean; ask only about ambiguous foreign bytes and require `WORKING_TREE_GATE OPEN`.
- Kimiflow's clarify gate uses `$KIMIFLOW_PLUGIN_ROOT/hooks/clarify-gate.sh`. Fresh Contract-4 features ask only missing material facts before planning, then require one receipt-bound final product-flow confirmation after PLAN exists. Existing schema-2/two-stage runs resume unchanged.
...
- The host hook records the final intake response. On resume call `status` once and trust it; never inspect receipt-hook internals or manufacture a receipt. Existing schema-2 runs continue by their reported stage/action.
- Before the final receipt, do not call `append-item` or mutate product files. The normal Codex plan widget and run-local planning artifacts are allowed; `phase-read` advances phase state mechanically.

...
Kimiflow inherits the session model but **allocates deliberately**: the session model (strongest available) keeps orchestration, planning, and Phase-2 understanding; under a Fable session the leaf seats route to Opus (leaf rule below — inert off the Fable family: a non-Fable session keeps its leaf seats on the session model); and a *different model family* breaks same-family blind spots. Routing is advisory allocation — never a gate, never a block; on hosts without per-subagent model selection everything simply inherits the session model.

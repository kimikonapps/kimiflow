...
**Reviewer rules:**
- **Fresh context, independent, adversarial framing.** Tell each reviewer: "you did NOT write this; assume it is flawed." On the first pass over a stable plan or code basis, each scheduled axis must enumerate every independently evidenced BLOCKER/HIGH it can establish in that bounded packet; it does not stop after the strongest objection or drip one material issue per round. MEDIUM/LOW never displaces material coverage. All same-round reviewers bind the same immutable PLAN hash and finish before any repair begins. Later reruns target complete repair families. The strongest de-biaser remains the **default cross-family lens** when available (→ "Model routing (per-role)").
...
- **Diverse lenses** (Phase 4 — canonical definitions; SKILL.md carries 1-line summaries):
  ...
  - **C — state, contract & aggregation:** enumerate legal/illegal/missing states; identity and evidence binding; lifecycle/time ordering and equality boundaries; classification/count partitions, derived formulas, denominators, thresholds, and verdict precedence. For profile=`contract`, try to falsify the pairwise `CONTRACT-MATRIX.json` and name uncovered combinations instead of checking prose linearly.
...
- **Plan saturation makes breadth and family closure mechanical.** Flow-schema-5 non-audit plans require STATE `Plan review contract: 1` and `Plan review profile: standard|contract`; Flow-schema≤4 remains resumable without them. Standard small reviews use exact lens IDs `a,b`; large/contract reviews use `a,b,c`. Flow-5 read-only audits intentionally retain their `AUDIT.md` basis and direct immutable findings grammar, but the resolver still enforces `a,b,c` and the same absolute three-round limit. A contract profile first passes `plan-review-gate.sh matrix`: its PLAN-bound schema-1 `CONTRACT-MATRIX.json` lists 1–12 dimensions, full legal/invalid/boundary assignments, invariant accept/reject pairs, classifications, operative `ACCEPTANCE.md` ownership, and expected outcomes; every pair of dimension values must occur in a case. For every contracted plan round, each candidate file starts `BASIS plan_sha256=<64hex>`, carries at least one `COVERAGE <domain> :: status=checked|not_applicable :: evidence=<ref>`, and then `NONE` or every `CANDIDATE ... :: family=<slug> :: verify=<typed method>`. Scheduled lenses collectively cover exactly `intent-trace,state-space,identity-binding,evidence-safety,time-lifecycle,aggregation-verdict,scope-subtraction`; a bare `NONE` is invalid. Schema-1 `plan-review-saturation/r<N>.json` binds PLAN, lenses, candidate/finding digests, one disposition per emitted BLOCKER/HIGH, and a complete root-family partition. Append exactly one immutable `kimiflow:plan-saturation` marker to `RECOVERY.md`, binding round, receipt digest, plan digest, and the previous receipt digest; every plan digest must equal the strategy baseline or a recovery `after` hash. `active-run.sh pin-plan-saturation --run <run> --round N --expect <lensCSV> --write` validates that full receipt and anchors the same chain in the locked Active-Run session state before the strict saturation gate. Disposition evidence is a one-line digest-pinned `REVIEW_EVIDENCE` receipt bound to the candidate method and reproduced/not-reproduced outcome; non-promoted candidates use deterministic class `candidate-<first54hex of candidate_id>`. Every family checks `upstream-requirements,sibling-states,downstream-outputs,boundary-values,failure-classification` with evidence; family identity cannot drift during resolution. Every later saturation call revalidates Active-Run pins, the append-only marker chain, all prior receipts, and bound artifact/evidence bytes. The gate rejects stale bases, rewritten history, narrowed topology, missing coverage, silent candidate drops, evidence/verifier mismatch, incomplete family sweeps, aggregate mismatch, and every material candidate in round 3.
...
**Autonomous recovery contract:** within the plan gate's fixed rounds 1–2 or the code gate's fixed rounds 1–3, a strategy change is allowed only after `RECOVERY.md` records one coherent falsifiable hypothesis plus a materially different strategy (evidenced root cause, algorithm/control flow, integration/architecture boundary, dependency choice, or AC-preserving task decomposition) and `PLAN.md` changes. Plan repair waits until all same-round candidates are verified, then changes every promoted root family in one coherent batch and refreshes the contract matrix before redispatch. A model switch, more tokens, rewording, whitespace, or file churn is not a strategy change; changed plan bytes are necessary, not sufficient. Each compact chronological entry stores gate/trigger/source/next round/cap, blocker classes, failed strategy/evidence, hypothesis/delta, fingerprints, outcome, and `<!-- kimiflow:recovery gate=<plan|code> source-round=<N> epoch-start=<S> cap=<C> before=<sha256> after=<sha256> -->`. Fingerprint/STATE/ledger continuity remains fail-closed. After two failed code strategies, trajectory preflight still requires a new falsifiable direction and cannot extend the four-round code limit. Plan closeout cannot extend the three-round plan limit; remaining material debt routes to decomposition or explicit follow-up. Semantic materiality remains reviewer/eval judgment, recorded as `promoted|refuted|non_blocking`.

...
**Anti-oscillation and global revision cap (blocker-aware):** compare the open BLOCKER/HIGH set round r→r+1. **Stop + ask with the gate CLOSED** if the count does not strictly decrease or a disappeared finding reappears. Round numbers are a run-global immutable ledger; Phase 4 never resets them across returns to Phase 3, and Phase 7 never resets them across fixes. Never overwrite/reset parsed `r<N>` files. Normal `small` feature/fix and `quick`/`small` code review use `--cap 2` (one repair); `large`/audit/release-critical uses `--cap 3` (two repairs). At the cap, clean may open and open findings emit `cap-reached`; any `round > cap` emits `cap-reached` even if its files say `NONE`. New user-requested scope after exhaustion starts an explicit new run, never a silent counter reset.

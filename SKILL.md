...
- **Pi/FirstMate:** Pi is Main. After `scope_ready`, `kimiflow_crew` may delegate bounded read-only `stage=research` Scouts; Ships and review Scouts require the final confirmed contract. FirstMate owns workers. A Ship pauses at a verified implementation checkpoint so Main can dispatch visible independent review Scouts. Workers use brief/status, not user dialogue or another Active Run.

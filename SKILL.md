...
8. **Project Map Bootstrap / status check** (recommended, never a prerequisite; full rules → reference.md "Project Map Bootstrap"). `PMS` = `${CLAUDE_PLUGIN_ROOT:-$CLAUDE_SKILL_DIR}/hooks/project-map-status.sh`. If the invocation is `--project-map quick|standard|deep`, create/update `.kimiflow/project/` at that depth, report paths, then STOP. If it is `--project-map skip`, record `project_map: skipped` in `STATE.md` and continue normally. Otherwise, on non-trivial runs, if `.kimiflow/project/INDEX.json` is missing, offer once: `standard` (recommended) / `quick` / `deep` / `skip`. If it exists, run `PMS status` (optionally with `--affected <path>` once Phase 2 knows likely paths) and record `project_map: current|partially_stale|stale|unknown`. If the affected section is `stale`/`potentially_stale`, offer a targeted delta refresh. For accepted standalone map/doc/improve runs, ask the focus and storage-target menus from reference.md; headless default is `codebase+architecture` focus and `kimiflow` storage. Decline, headless, no answer, `unknown`, or `skip` → continue unchanged. If accepted, run bootstrap/refresh before Phase 2 so later understanding can reuse the map.

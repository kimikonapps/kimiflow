...
- Phase-7 Review Ensemble runs every axis once per stable PLAN, then path-required repair deltas without calibration. Limit: three semantic rounds + one resolution-only closeout. Critical paths keep required axes. Verify each candidate as `promoted|refuted|non_blocking`.

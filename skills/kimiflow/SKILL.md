...
- `/kimiflow full|grill|plan|build|quick|review|audit|fix` maps directly to Codex. `full` forces the strict loop and one mode-specific Preview approval; `grill|plan|review|audit` are no-code modes until later approval/selection; `quick` uses compact feature intent confirmation and normally a no-worker Discovery pulse; `fix` uses one post-diagnosis Fix Preview instead of an early confirmation plus Build Preview; `review` maps to existing-feature/current-change review.
...
- Kimiflow's clarify gate uses `$KIMIFLOW_PLUGIN_ROOT/hooks/clarify-gate.sh`. Feature/audit small/quick requires current-run confirmation of behavior, scope, and outcome with no minimum question count. Fix Phase 1 needs only the problem brief; after diagnosis the one Fix Preview runs `--record-fix-approval`, and its basis-bound approval must pass `--post-diagnosis` before Phase 5.

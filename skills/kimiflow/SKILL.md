...
- `kimiflow security scan <path>` / `kimiflow security diff <path>` writes private local evidence without model calls or publication.
...
- `$KIMIFLOW_PLUGIN_ROOT/hooks/workspace-preflight.sh` inventories/routes: clean/free primary stays direct; dirty/busy gets one locked owned `codex/<slug>*` tree; later runs queue FIFO. Switch to its returned root before STATE/Active Run. Phase 3 `declare`s paths/contracts against PLAN; Phase 5 requires `write-gate`. Commit, `integrate` from primary with JSON argv checks, finish in the task root, then `retire` from primary. Never mutate manual or `${CODEX_HOME:-~/.codex}/worktrees` trees, nor ask commit/stash/clean for routine routing. Ask once only for ambiguous foreign bytes; require `WORKING_TREE_GATE OPEN` in the selected root.

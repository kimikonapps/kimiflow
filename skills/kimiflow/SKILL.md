...
- Phase-7 Review Ensemble pins one per-path basis and runs all axes first. Only an exact model/prompt/runtime evidence-calibrated lease may narrow post-repair axes; changed fingerprints, risky/broad deltas, and audits force full review. Candidates require orchestrator verification before becoming canonical findings.
...
- `$KIMIFLOW_PLUGIN_ROOT/hooks/workspace-preflight.sh` inventories/routes: clean/free primary stays direct; dirty/busy gets up to three locked owned `codex/<slug>*` Fleet trees; later runs queue FIFO. Switch to its returned root before STATE/Active Run. Phase 3 `declare`s exclusive Primary/Fleet path-contract leases against PLAN and reports `blocked_by`; Phase 5 requires `write-gate` and `revalidate` after Primary advances. Commit, `integrate` the exact combined candidate from primary with JSON argv checks, recover `needs-reconcile` or finish in the task root, then `retire` from primary. Never mutate manual or `${CODEX_HOME:-~/.codex}/worktrees` trees, nor ask commit/stash/clean for routine routing. Ask once only for ambiguous foreign bytes; require `WORKING_TREE_GATE OPEN` in the selected root.
- Kimiflow's clarify gate uses `$KIMIFLOW_PLUGIN_ROOT/hooks/clarify-gate.sh`. New Contract-3 non-trivial features require one explicit bounded Product Intake before planning or writes, forbid technical HOW questions, and use a bounded critic for large/critical work. At most one causal second batch is allowed when the first answer creates a new material product conflict. Content-free receipts and an Active-Run-pinned intent lock preserve privacy and scope; older runs, audits, fixes, trivial work, and schema 3 retain compatible contracts.

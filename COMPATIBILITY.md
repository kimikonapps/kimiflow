...
| Task/version-bound `UserPromptSubmit` observation | `active-run.sh prompt-context` records a content-free runtime observation; `hook-health --require` exposes it diagnostically while Product Intake arms its owner-bound wait first | **Load-bearing for intake completion, not wait registration** — the next direct owner prompt or explicit native response binds to the pending request digest and writes the receipt. A missing observation leaves the wait pending and the Intent Lock closed; only a changed plugin or hook manifest reports `restart_codex_and_start_new_task` |
...
| Command-adapter protocol v1 (`capabilities --json`, JSON-stdio `start|resume`) | tool-capable local/remote coding-agent harnesses, including local models and optional app hosts | **Load-bearing only for a non-Codex terminal adapter** — all five capabilities (`files`, `shell`, `tests`, `resume`, `gates`) are mandatory; app-host features are negotiated and absent by default. Optional `adaptive_execution_profiles` exposes only a model fingerprint, opaque host variants/limits/control support and exact Usage-v2 counters—never provider names or a fixed effort vocabulary. Invalid/oversized/profile-mismatched events fail closed; feature-capable resume requires the identical privacy-safe contract fingerprint. Embedded Codex/Claude and legacy command payloads remain unaffected |
| MCP `2025-11-25` newline-delimited stdio | optional KimiTalk/IDE/local-host facade over the canonical Run Bridge | **Load-bearing only for MCP clients** — protocol drift needs a transport-only adapter; missing owner identity blocks mutations but not reads. The embedded plugin, CLI runner and Run Bridge remain independent |
| Optional code-intelligence command provider v1 | bounded typed code relationships for evidenced `large` cross-file runs | Graceful — absent/ineligible/stale/dirty-incompatible/unsafe/slow provider → existing Project Map/`rg` route with no user gate. No provider process or context overhead exists on the normal small/local path |
...
| Allowlisted marketplace candidate + `RUNTIME-FINGERPRINT.json` | publish-safe plugin contents and same-version cache freshness | **Load-bearing for releases** — inventory/content/executable-bit drift blocks the manual release check; private maintainer/run data and maintainer-only release tooling are never copied into the candidate |
| Deterministic runtime ZIP + update-manifest v1 + GitHub immutable releases | independent installation/update by KimiTalk or another app host without a Kimiflow fork | **Load-bearing only for external-host updates** — offline/draft metadata can prove artifact integrity but never official compatibility; activation requires fixed-origin release/tag reads, matching SHA-256 assets/source commit, an immutable release, supported adapter protocol, and all features of the selected `embedded` or `app_host` profile. Embedded marketplace use remains independent |

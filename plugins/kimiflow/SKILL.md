...
- **Workspace + Active Session Contract.** `workspace-preflight.sh`: free primary direct; busy/dirty gets up to three locked `codex/<slug>*` Fleet trees; then FIFO. Foreign/Codex untouched. Phase 3 binds exclusive Primary/Fleet path-contract leases to PLAN and reports `blocked_by`; Phase 5 gates writes and requires `revalidate` after Primary advances. Commit → combined merge-tree/argv integration → recoverable `needs-reconcile` or finish → ancestry retire. `working-tree-gate.sh` uses its root.
...
Details: `docs/kimiflow-scaling-knobs.md`. Display verbosity changes output only, never gates/cost/quality. Solo-dev uses one implementation path per run and at most three managed Fleet trees; behavioral evals never run in CI.

...
- **Artifact economy.** Write artifacts as required fields + evidence, without narration or padding; retain every locator and precise acceptance oracle. Before decision-bearing prose, read all of `${CLAUDE_PLUGIN_ROOT:-$CLAUDE_SKILL_DIR}/references/workflow-prose-quality.md` (Codex: `$KIMIFLOW_PLUGIN_ROOT/references/workflow-prose-quality.md`) and apply it in the same model pass; semantic defects stay with existing gates and style alone never blocks.
...
- **Agent budget.** Default 1 implementer + 1–2 reviewers; fan out ~5–10 only when useful, >10 asks first. Record fan-out; substitutions reuse a seat and external CLI counts as one. Fold work unless independence matters; research counter-perspectives and semantic review are independence work, not implementer self-review.
...
- **Phase 3 protected rules:** one flat minimum-complete, subtracted, AC-mapped plan. Material decisions declare `review_only|spike_required|runtime_required`; required spikes are digest-bound. Contract 1 adds checkable slices/failure classes. New reviews mark `standard|contract`; contract plans bind a pairwise state matrix to PLAN. Managed trees bind paths/contracts to its digest.
- **Phase 4 held rule:** only evidenced BLOCKER/HIGH revises. All lenses share one frozen PLAN; complete coverage and family-wide checks precede one batch repair. Rounds are globally fixed: discovery, repair verification, resolution-only closeout; PLAN/model/strategy changes never reset them. Architecture changes need an executable failure/named-invariant violation; schema 4+ pauses only for material risk.

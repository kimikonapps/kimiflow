...
- Large `full|plan` after final `confirmed`: one Sol `fork_turns="none"` worker replaces parent planning through Phase 4; the parent waits once for `READY_PHASE5|NEEDS_DECISION`. Phase 4 dispatches scheduled lenses in parallel on one frozen PLAN hash, seals and repairs the whole batch, and never resets the three-round cap.

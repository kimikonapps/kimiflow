...
def runtime_manifest_valid(runtime_root, manifest):
    ...
    if not isinstance(expected, str) or re.fullmatch(r"sha256:[0-9a-f]{64}", expected) is None:
        return False
    if not isinstance(rows, list) or not rows or len(rows) > 512 or manifest.get("file_count") != len(rows):
        return False
    ...
    for row in rows:
        ...
        if total > 16 * 1024 * 1024:
            return False
        path = os.path.join(runtime_root, *rel.split("/"))
        flags = os.O_RDONLY
        ...
        try:
            ...
            try:
                ...
                if not stat.S_ISREG(info.st_mode) or info.st_size != size:
                    return False
                actual_mode = "0755" if stat.S_IMODE(info.st_mode) & 0o111 else "0644"
                if actual_mode != mode:
                    return False
                chunks = []
                remaining = size + 1
            ...
            return False
        if len(payload) != size or "sha256:%s" % hashlib.sha256(payload).hexdigest() != content_hash:
            return False
    ...
    return "sha256:%s" % digest.hexdigest() == expected
...
def build_snapshot(root, script_dir):
    ...
    installed_runtime = load_json_file(os.path.join(script_root, "RUNTIME-FINGERPRINT.json"))
    expected_runtime = load_json_file(os.path.join(root, "plugins/kimiflow/RUNTIME-FINGERPRINT.json"))
    installed_fingerprint = installed_runtime.get("runtime_fingerprint", "unknown") if isinstance(installed_runtime, dict) else "unknown"
    expected_fingerprint = expected_runtime.get("runtime_fingerprint", "unknown") if isinstance(expected_runtime, dict) else "unknown"
    installed_runtime_valid = runtime_manifest_valid(script_root, installed_runtime) if isinstance(installed_runtime, dict) else None
    expected_runtime_root = os.path.join(root, "plugins", "kimiflow")
    expected_runtime_valid = runtime_manifest_valid(expected_runtime_root, expected_runtime) if isinstance(expected_runtime, dict) else None
    ...
    if installation_mode == "plugin_cache":
        ...
        elif installed_runtime_valid is False or expected_runtime_valid is False:
            cache_status = "stale_cache"
            installation_action_required = True
        elif expected_fingerprint != "unknown" and installed_fingerprint == "unknown":
            cache_status = "stale_cache"
            installation_action_required = True
        elif installed_fingerprint != "unknown" and expected_fingerprint != "unknown" and installed_fingerprint != expected_fingerprint:
            cache_status = "stale_cache"
            installation_action_required = True
    ...
    snapshot = {
        ...
        "installation": {"mode": installation_mode, "plugin_root": script_root, "version": plugin_version, "repo_version": repo_plugin_version, "runtime_fingerprint": installed_fingerprint, "repo_runtime_fingerprint": expected_fingerprint, "cache_status": cache_status, "action_required": installation_action_required},
    ...
    snapshot["launcher"] = {
        ...
        "status": {
            "installation": {"version": snapshot["installation"]["version"], "repo_version": snapshot["installation"]["repo_version"], "runtime_fingerprint": snapshot["installation"]["runtime_fingerprint"], "repo_runtime_fingerprint": snapshot["installation"]["repo_runtime_fingerprint"], "mode": snapshot["installation"]["mode"], "cache_status": snapshot["installation"]["cache_status"], "action_required": snapshot["installation"]["action_required"]},
            "project_map": {"present": snapshot["project_map"]["present"], "status": snapshot["project_map"]["status"], "attention": (snapshot["project_map"]["present"] is not True or snapshot["project_map"]["valid"] is not True or snapshot["project_map"]["status"] != "current")},
            ...
            "counts": {"findings_open": findings_open, "active_runs": active, "backlog_runs": backlog},
        },

...
def _compile_descriptor(root, run_descriptor, phase):
    ...
    for reference in phase_reads.reference_section_records(entry):
        if reference["size"] > policy["max_file_bytes"]:
            raise PhaseContextError("reference_section_oversize:%s" % reference["name"])
    ...
    required = set(policy["required"]) | set(policy.get(mode, []))

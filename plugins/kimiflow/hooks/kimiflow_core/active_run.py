...
def normalized_host(value):
    ...
    return normalized if re.fullmatch(r"[a-z0-9][a-z0-9._-]{0,31}", normalized) else ""
...
def record_host_usage(root, status, usage=None):
    """Persist normalized host counters, or explicit unavailability, without guessing zero."""
    run_dir = resolve_run_dir(root, status["run"])
    path = os.path.join(run_dir, HOST_USAGE_RECEIPT)
    existing = None
    ...
    if os.path.lexists(path):
        ...
        try:
            ...
            info = os.fstat(descriptor)
            if not stat.S_ISREG(info.st_mode) or info.st_size > MAX_HOST_USAGE_BYTES:
                existing_invalid = True
            else:
                payload = os.read(descriptor, MAX_HOST_USAGE_BYTES + 1)
                if len(payload) > MAX_HOST_USAGE_BYTES:
                    existing_invalid = True
                else:
    ...
    if isinstance(existing, dict) and existing.get("status") == "available":
        existing_invalid = any(
            isinstance(existing.get(key), bool)
            or not isinstance(existing.get(key), int)
            or existing.get(key) < 0
            for key in execution_control.USAGE_KEYS
        )
    elif isinstance(existing, dict) and existing.get("status") == "unavailable":
        existing_invalid = any(existing.get(key) is not None for key in execution_control.USAGE_KEYS)
    ...
    if usage is None:
        value = {
            "schema_version": 1,
            "status": "unavailable",
            **{key: None for key in execution_control.USAGE_KEYS},
        }
    else:
        counters = {}
        for key in execution_control.USAGE_KEYS:
            ...
            if isinstance(item, bool) or not isinstance(item, int) or item < 0:
                raise ActiveError("host usage counters invalid", 2)
            counters[key] = item
        if existing_invalid or (isinstance(existing, dict) and existing.get("status") == "unavailable"):
            value = existing
            if existing_invalid:
                value = {
                    "schema_version": 1,
            ...
            previous = existing if isinstance(existing, dict) and existing.get("status") == "available" else {}
            value = {
                "schema_version": 1,
                "status": "available",
                **{key: int(previous.get(key, 0)) + counters[key] for key in execution_control.USAGE_KEYS},
            }
    atomic_write(path, json_pretty(value) + "\n", mode=0o600, refuse_symlink=True)
    return value


def headless_controller_owns_turn(root, status):
    """Recognize the runner receipt so its outer turn is observed exactly once."""
    if os.environ.get("KIMIFLOW_RUNNER_CONTROLLER") != "1":
        return False
    path = os.path.join(root, ".kimiflow", "session", "HEADLESS_RUN.json")
    descriptor = None
    try:
        flags = os.O_RDONLY
        if hasattr(os, "O_NOFOLLOW"):
    ...
    return (
        ...
        and os.path.realpath(str(receipt.get("root", ""))) == os.path.realpath(root)
        and receipt.get("host") == owner.get("host")
        and receipt.get("session_id", receipt.get("thread_id")) == owner.get("session_id")
    )
...
def cmd_start(args):
    ...
    if schema4_state:
        ...
        selected_scope = str(active_scope or "").strip().lower()
        if not persisted_mode or persisted_mode != selected_mode:
            die("schema-4 run mode selector does not match STATE", 1)
        if not persisted_scope or persisted_scope != selected_scope:
            die("schema-4 run scope selector does not match STATE", 1)
    flow_schema = persisted_flow_schema
...
def cmd_stop_gate():
    ...
    if execution_control.contract_enabled(load_active(root)):
        try:
            if not headless_controller_owns_turn(root, status):
                record_host_usage(root, status, None)
                _observe_execution(
                    root,
                    status,
                    "turn_completed",

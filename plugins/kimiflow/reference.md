...
- **Repair batching replaces finding-by-finding patching.** Only aggregate promoted BLOCKER/HIGH classes enter `review-repairs/r<N>.json` with exact round, current plan/findings SHA-256 and bounded root-cause groups. Schema 2 additionally binds the exact source-saturation SHA-256 and is required only when that repair must authorize the plan-changing full-closeout exception; schema 1 remains valid for ordinary repair/delta flows. Each open class appears exactly once, dependencies are acyclic, and at least one meaningful typed check per group reuses authoritative finding evidence. Refuted/non-blocking/material-decision outcomes never enter repair.

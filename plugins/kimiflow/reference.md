...
hooks/active-run.sh await-user --run .kimiflow/<slug> --kind intake --round 1 --request .kimiflow/<slug>/INTAKE.md --reason scope_deliberation --write
hooks/active-run.sh await-user --run .kimiflow/<slug> --kind intake --round 2 --request .kimiflow/<slug>/INTAKE-2.md --reason final_contract --write
...
- **Discovery worker budget:** `none` spawns no research worker. Outside an active Pi/FirstMate crew, `pulse` normally stays in the orchestrator and `focused` normally uses one `cheap|balanced` worker, with at most two in parallel for explicitly independent lanes. With an active Pi/FirstMate crew, Kimiflow Main—not Captain—uses at least one visible read-only `stage=research` Scout for every `pulse|focused` pass so Main has an independent evidence perspective; use exactly two only for genuinely separate lanes. A selective `top|cross_family_top` countercheck may replace one quality seat for security/privacy/auth/payment/public-contract/migration/lock-in/high-cost/immature-tech decisions; it tries to refute the chosen option rather than repeating broad research.
...
- **Context rollover:** Phase Context compares the previous and current digest-bound shadow. A rollover is
  pending only under measured hard pressure (`pressure=hard` or at least 120k observed cumulative input tokens),
  never merely because a large run entered another phase. The adapter must negotiate `context_rollover`; its next resume request receives only
...
- **Pi/FirstMate review topology:** an active visible crew overrides `embedded` for semantic code review. The implementation Ship pauses at an exact committed review-ready checkpoint; Kimiflow Main—not Captain—sends the frozen packet to one Scout for `quick`, two axis Scouts for `small`, and three axis Scouts for `large`/release-critical. Main verifies their candidates and steers the same Ship. Only unavailable Main crew capability permits a recorded folded fallback.
- **Repair-delta review:** the first review round always saturates every scheduled axis. Every later round on
  ...
  including for critical work. `review-convergence-gate.sh delta` binds the previous schema-4 saturation, exact
...
Inspect in strict order: **reuse → evolve → new**. `RESEARCH.md` binds the exact basis-file digest and selected scope digest, compares `Own idea`, `Research finding`, and `Code comparison`, and records `Scope result: non_expanded`. Reuse/evolve evidence must cite a current regular repo source. The only coherent marker routes are `reuse=fit evolve=not_needed new=not_needed selected=reuse`; `reuse=gap evolve=fit new=not_needed selected=evolve`; or, only with both proven gaps and a typed falsifier, `reuse=gap evolve=gap new=selected selected=new`. A new file alone does not select `new` when the behavior evolves an existing boundary. The marker is `<!-- kimiflow:reuse-order contract=4 schema=2 reuse=fit|gap evolve=fit|gap|not_needed new=selected|not_needed selected=reuse|evolve|new -->`; the scope marker is `<!-- kimiflow:scope-research contract=4 schema=2 codebase_basis=sha256:<basis-file-digest> scope=sha256:<selected-scope-digest> selection=non_expanded -->`. Research challenges technical HOW and the model's first idea; it never expands the user's WHAT.
...
- For `large`, run `memory-router.sh status > .kimiflow/<slug>/MEMORY-STATUS.json` and read only bounded decision fields when a usable pre-run `.kimiflow/project/` memory/facts/history/index source exists. With none, record the skip instead of materializing a full empty status. Then read `.kimiflow/project/MEMORY.md` only if present and under budget.
...
Bounded, local-first project brain under `.kimiflow/project/` (**project source of truth**) plus an independent
human-readable cross-project store under `KIMIFLOW_HOME/memory` (default `~/.kimiflow/memory`) — no API key,
subscription, MCP server or Obsidian required; providers are optional and graceful. Run artifacts live under
`.kimiflow/<slug>/` and are searched as local history.
...
source-preference order (`facts`, `learnings`, `global_notes`, `strategies`, `history`, `index`) before the remaining unique
...
**Human-readable global memory:** only a new explicit probationary→durable lifecycle transition may create a
global note, and only after the existing Privacy Capsule `portable_entry` gate accepts the exact row. Missing
legacy maturity never triggers export. The lifecycle stores a small pending operation inside the existing local
`curation` object, commits the trust transition, completes the global operation under one lock, then clears the
marker. A retry processes only that marker. This closes ordinary failure windows without treating every durable
row as repair work; a manually deleted note therefore stays deleted and a human edit is never overwritten.

Each insight is one canonical mode-0600 UTF-8 Markdown file named by its stable `cap_<sha256>` content ID under
`memory/notes/`. Its fixed human-readable metadata, Learning section and bounded Related section use ordinary
relative Markdown links to already existing related canonical notes. `memory/INDEX.md` is generated navigation,
not canonical knowledge. A separate human-readable Markdown graph under `memory/bindings/` stores one opaque,
create-only edge per project learning and links it to the canonical note. The edge ID binds the physical project
root, local learning ID, learning fingerprint and content ID without exposing those project-local values. Shared
content therefore keeps one note until its last independent edge is revoked; copying public note metadata cannot
authorize another project's revocation. Create is exclusive and create-only; promotion/revocation and index refresh
pin no-follow `memory`/`notes`/`bindings` directory descriptors, roll the index back after any failed durable
replacement, and fail closed if a parent is a symlink or changes identity.

Recall performs a bounded scan of at most 500 regular non-symlink notes of at most 32 KiB each. Strict current
metadata, freshness and the existing prompt-injection/secret/URL/path/private-content policy are rechecked after
every human edit. Malformed, inactive, stale, unsafe, oversized, symlinked or concurrently changed notes are
ignored without aborting recall. Only notes with at least one valid binding edge are recalled; the binding scan is
independently capped at 2,000 regular Markdown edges. The generated index has a separate 128-KiB bound sufficient for all 500 notes,
and a full store rejects another note before changing either the note set or index. Accepted `global_notes` are
ranked by query coverage before their bounded candidate window enters the same global hit/token packer below current
project facts and learnings, so local duplicate content wins. Every selected global hit has a deterministic Recall-ID
and a standard Markdown source link to its canonical note. `status` reveals only the display path and valid/ignored
counts with aggregate reasons—never note text or project provenance. No database, daemon, provider sync, migration
or legacy backfill is involved.
...
reviewable provider handoff rather than an automatic provider import; the built-in global Markdown store follows
the separate verified lifecycle transition above.
...
use, absent → record one bounded skip + continue (no provider is ever required). Broad recall is `scope=large`
only when a usable pre-run project/global source exists; redirect its machine JSON while writing and read only
the bounded `RECALL.md`. Probe Vault health only when a provider manifest or direct Vault tool actually exists.
`small`/`quick` skip both unless the explicit prior-work cue below applies. The learning loop (Phase 7
`review-run`) still runs at every scope.
...
.kimiflow/<slug>/RECALL.md >/dev/null`. The targeted flag excludes always-on/user memory, facts, and the FTS index, then caps
...
**Vault Pulse (`scope=large` only, configured/connected only):** when `.kimiflow/project/VAULT-PROVIDER.json`
exists or a direct Vault tool is connected, run `memory-router.sh provider health >
.kimiflow/<slug>/PROVIDER-HEALTH.json` before web/current-source research and inspect only its status/capability
fields. With neither, record `Vault pulse: skipped_no_provider` and make no provider call.
...
```text
hooks/current-state-gate.sh assess --input <INTENT.md|PROBLEM.md|AUDIT-INTENT.md> --write
...
`assess --write` atomically writes sibling `CURRENT-STATE.json` and prints one compact risk/path receipt.
Without `--write`, it retains the JSON preview form for diagnostics and compatibility.
...
|---|---|---|
| `low` | local code/docs work or stable project convention, including a public in-repo API, current store state, or compatibility promise | persist the low assessment; no freshness browsing; does not suppress a later named Discovery/Reference Strategy Fit gap |
| `medium` | an explicitly current/changing dependency, platform, external API, or method may have changed | fresh memory/vault hit or one short primary-source check required before spec/plan finalization |
...
**Reviewer rules:**
- **Fresh context, independent, adversarial framing.** Tell each reviewer: "you did NOT write this; assume it is flawed." On the first pass over a stable plan or code basis, each scheduled axis must enumerate every independently evidenced BLOCKER/HIGH it can establish in that bounded packet; it does not stop after the strongest objection or drip one material issue per round. MEDIUM/LOW never displaces material coverage. All round-1 reviewers bind the same immutable PLAN hash and finish before any repair begins. Later plan reruns use one fresh independent whole-contract verifier for every repaired family, its counterexamples, and all coverage domains; it writes the exact lens partitions for the seal without pretending they are separate opinions. Code review keeps its own topology. The strongest round-1 de-biaser remains the **default cross-family lens** when available (→ "Model routing (per-role)").
...
- **Diverse lenses** (Phase 4 — canonical definitions; SKILL.md carries 1-line summaries):
  ...
  - **C — state, contract & aggregation:** enumerate legal/illegal/missing states directly from intent and acceptance; test identity/evidence binding, lifecycle/time ordering and equality boundaries, classification/count partitions, derived formulas, denominators, thresholds, and verdict precedence. For profile=`contract`, name uncovered combinations explicitly instead of requiring the planner to prebuild a model-authored cross-product matrix.
...
- **Compact plan exception:** under `Plan review contract: 1`, the scheduled reviewer may emit BLOCKER/HIGH only after applying the stated method and independent-evidence threshold. `plan-review-gate.sh seal` is then the sole promoter and evidence/receipt writer; the orchestrator does not recreate or hash those administrative artifacts. The next independent frozen-basis round supplies negative resolution. Contracted code review retains the explicit reproduce/disposition path below.
- **Plan saturation makes breadth and family closure mechanical without model-authored administration.** Flow-schema-5 non-audit plans require STATE `Plan review contract: 1` and `Plan review profile: standard|contract`; Flow-schema≤4 remains resumable without them. Standard small reviews use exact lens IDs `a,b`; large/contract reviews use `a,b,c`. Contract selects the independent state/aggregation lens; it does not require `CONTRACT-MATRIX.json`. Flow-5 read-only audits retain their `AUDIT.md` basis and direct immutable findings grammar, but the resolver still enforces `a,b,c` and the same absolute three-round limit. For every contracted plan round, each candidate file starts `BASIS plan_sha256=<64hex>`, carries at least one `COVERAGE <domain> :: status=checked|not_applicable :: evidence=<ref>`, and then `NONE` or every candidate as `CANDIDATE <severity> <ref> :: <claim> :: family=<slug> :: verify=<typed method>`. Scheduled lenses collectively cover exactly `intent-trace,state-space,identity-binding,evidence-safety,time-lifecycle,aggregation-verdict,scope-subtraction`; a bare `NONE` is invalid. Once all candidate files exist, `plan-review-gate.sh seal --run <run> --round <N> --expect <lensCSV> --write` deterministically generates the digest-bound findings/evidence, root-family groups, schema-1 saturation receipt, recovery chain, Active-Run pin, validation, and binary resolver result in one host call. BLOCKER/HIGH candidates are authoritative only under the reviewers' independent-evidence threshold; MEDIUM/LOW remain advisory. On the next frozen basis, absence of a prior family generates its exact negative-resolution evidence. Every later seal revalidates all historical receipt and artifact bytes, topology, coverage, bases and family identity. Round 3 rejects every material candidate, and the global limit remains absolute.
- **Code saturation is mechanically bounded to emitted evidence and stable bytes.** For every contracted code round, run `review-convergence-gate.sh basis --run <run> --base <validated-base> --details` and copy its exact base/target/snapshot plus per-path `review_files` into mandatory schema-4 `review-saturation/r<N>.json`, together with exact `round`, current `plan_sha256`, `scheduled_axes`, actual `axes`, each scheduled candidate file's SHA-256, one exact `code-review-cascades/r<N>.md` path+digest, `carried_classes`, and one disposition per emitted BLOCKER/HIGH candidate. Only the first round for a stable PLAN may set `delta_receipt=null` and actual axes equal scheduled axes; a second full receipt on the same PLAN closes. The snapshot covers HEAD plus every STATE-declared affected path including file type/existence, so any later source mutation makes that receipt stale. Candidate identity is `cand_` plus SHA-256 of `axis + NUL + exact candidate line`; scheduled and cascade-scan candidates use the same grammar and disposition path. Multiple axes may map the same underlying class only when verify/evidence/cascade agree; the aggregate carries that class once. A carried class must match the immediately preceding aggregate's verify/evidence identity exactly. An incremental repair round requires `review-convergence-gate.sh delta` to validate the exact previous schema-4 saturation and source-bound schema-3 repair digests, unchanged PLAN, per-path repair delta, and path-required axes; the resulting `review-deltas/r<N>.json@<sha256>` becomes `delta_receipt`. Run `review-convergence-gate.sh saturation --run <run> --round N --axes <actual-csv>` before the resolver; contracted code resolution repeats it with the same actual axes. Missing axes, changed source/candidate/cascade/plan/route bytes, unsafe or over-broad deltas, undisposed material candidates, forged/drifted carry-forward, mixed outcomes, or aggregate mismatch close. Round 4 forbids every emitted candidate and every carried class and normally requires delta-bound individual negative evidence. The one authenticated plan-changing Round-3 recovery exception accepts only the fully revalidated schema-4 source, its exact schema-3 repair, exact source axes, and individual negative evidence bound to both the repair and current closeout saturation receipt. Unchanged-plan, narrowed-axis, precomputed/evidence-free, verifier-drifted, invalid-source, source-drifted, malformed-recovery and missing-repair variants remain closed. Saturation schema 1–3, delta/recovery sources below schema 4, and repair schema 1/2 are rejected unconditionally; there is no legacy receipt bypass. This proves axis completion and disposition of every emitted material candidate; it cannot prove a model noticed an un-emitted latent defect.
- **Schema-4 materiality and cascade disposition.** Schema 4 retains the basis/delta and schema-3 materiality fields and adds exactly one `cascade` field. Every promoted class carries one stable root ID/cause/assumption, one role `root|upstream|sibling|downstream`, and exactly one typed-evidence `checked|not_applicable` probe for each `direct-callers,data-flow,shared-state,assumption-users,error-consequences` surface. At most 32 promoted classes are accepted; every group has identical cause/assumption, exactly one root and at least one scheduled-review origin. Non-promoted rows require `cascade:null`. Status values remain `violated|not_violated`, `supported|unsupported`, and impact `none|correctness|runtime|security|privacy|data_loss|paid|scope|breaking|irreversible`. `promoted` requires a reproduced supported contract violation with concrete impact and is the only repair input. `refuted` requires `not_reproduced`. `non_blocking` requires reproduced evidence, `not_violated`, `unsupported`, `none`, and a bounded proportionality rationale; security/privacy/data-loss/irreversible impact cannot be waived. `material_decision` requires a reproduced supported contract defect whose repair crosses `paid|privacy|scope|breaking|irreversible`; saturation returns `CLOSED/material-decision-required`, direct repair also remains closed, and the existing typed user gate must resolve authority before replacement promotion or park/fail. A `runtime` class may use text-only `verifier:` evidence twice; its third same-class disposition requires executable `command:` evidence.
- **Repair batching replaces finding-by-finding patching.** Only aggregate promoted BLOCKER/HIGH classes enter a source-bound schema-3 `review-repairs/r<N>.json` with exact round, current plan/findings SHA-256, exact schema-4 source-saturation SHA-256 and bounded root-cause groups. Group IDs, root-cause text and class sets exactly match the authenticated cascade groups; every member's authoritative typed verifier remains an explicit check. Each open class appears exactly once and dependencies are acyclic. Refuted/non-blocking/material-decision outcomes never enter repair; schema-1/2 repair receipts are rejected.
...
- **Class-scoped trajectory schema 2.** When the latest two failed recovery rounds use mandatory schema-4 saturation, preflight intersects their promoted stable classes. A→B does not cross-count and stays below threshold; A→A requires `stable_class:A` plus exact `Trajectory class: A` in PLAN. Only same-class prior trajectory hashes are bound. The first A reset needs a new falsifiable hypothesis/assumption/typed check; any later A reset must materially change the action or check identity, so rewording cannot reopen the loop.
- **Mechanical plan-blocker gate (Phase 4, before reviewers).** Run `${CLAUDE_PLUGIN_ROOT:-$CLAUDE_SKILL_DIR}/hooks/plan-blocker-gate.sh .kimiflow/<slug>` only after the Phase-4 read. It re-runs Clarify and Discovery, verifies the current Contract-4 codebase basis, then blocks unresolved markers, unmapped ACs, missing verification/path evidence, undeclared affected files, and malformed Contract-1 Architecture Deliberation shape/budgets/linkage. `PLAN_BLOCKER_GATE	OPEN	blockers=0	reason=clean` is required before round 1 and once after every batched plan repair.
...
- **Gate count (mechanical, current round only) — delegated to the tested resolver.** The orchestrator runs `${CLAUDE_PLUGIN_ROOT:-$CLAUDE_SKILL_DIR}/hooks/resolve-review-gate.sh .kimiflow/<slug>/findings --round <N> --expect <lensCSV> --gate <plan|code> --epoch-start <S> --cap <C>`, adding `--finding-contract 1` when STATE declares Convergence Contract 1 and, for contracted code, `--review-axes <csv>`. Every Flow-schema-5+ plan review has one global absolute `C=3` across every strategy, basis hash, model, and recovery: round 1 is exhaustive discovery, round 2 verifies the batched family repair plus counterexamples, and round 3 is resolution-only closeout; Flow-schema≤4 markerless runs retain their legacy epoch behavior. Flow-5 audits use `AUDIT.md` as the strategy basis and exact lenses `a,b,c`; other current plans require the Plan Review Contract and its saturation gate. Contracted code retains global absolute `C=4`: rounds 1–3 discover and round 4 closes, and a persisted plan selector never changes a code-gate call into a plan call. Before each gate's round 1, `RECOVERY.md` gets exactly one `<!-- kimiflow:strategy gate=<plan|code> epoch-start=1 fingerprint=<sha256(applicable basis)> -->` marker. New plan-review contracts require explicit `--gate plan --epoch-start <S> --cap 3 --finding-contract 1` and repeat `plan-review-gate.sh saturation`; contracted code requires the analogous code options plus `--review-axes`. The resolver canonicalizes Flow schema, validates completeness, grammar, evidence safety/digests, negative resolution, saturation, and strategy liveness, and hard-closes current plan `N>3` or code `N>4` with `review-limit-reached`. Only `OPEN/clean` advances. Transport errors repair the same round; open material plan debt may change strategy only inside the remaining global rounds. `review-limit-reached` preserves blockers for AC-preserving decomposition or an explicit follow-up decision; it never starts another automatic epoch. The resolver remains language-agnostic, tested, and independent of `REVIEW.md`.
...
**Anti-oscillation and bounded strategy recovery (blocker-aware):** each plan/code gate keeps its own global, monotonically increasing findings ledger; never overwrite/reset a grammar-valid `r<N>` file. Inside an allowed strategy epoch, open BLOCKER/HIGH count must strictly decrease and a disappeared finding may not reappear. Under Convergence Contract 1, the same stable root class reproduced in consecutive rounds emits `root-class-repeated` even when the ref moved. These are cheap liveness signals: they can force a strategy change but never extend a contracted gate. Flow-schema-5 plan review has two semantic rounds plus resolution-only round 3; code review has three semantic rounds plus resolution-only round 4. Those limits span every strategy/model/PLAN change. A new material plan candidate in round 3, every contracted plan `N>3`, every code `N>4`, or unresolved material debt at closeout emits a closed limit verdict; no epoch, model switch, rewording, or clean file resets it.

**Autonomous recovery contract:** within the plan gate's fixed rounds 1–2 or the code gate's fixed rounds 1–3, a strategy change is allowed only after `RECOVERY.md` records one coherent falsifiable hypothesis plus a materially different strategy (evidenced root cause, algorithm/control flow, integration/architecture boundary, dependency choice, or AC-preserving task decomposition) and `PLAN.md` changes. Plan repair waits until all same-round candidates arrive, then changes every material root family in one coherent batch before redispatch; the compact plan seal records and binds that transition mechanically. A model switch, more tokens, rewording, whitespace, or file churn is not a strategy change; changed plan bytes are necessary, not sufficient. Fingerprint/STATE/ledger continuity remains fail-closed. After two failed code strategies, trajectory preflight still requires a new falsifiable direction and cannot extend the four-round code limit. Plan closeout cannot extend the three-round plan limit; remaining material debt routes to decomposition or explicit follow-up.
...
The Pi session in which the user invokes Kimiflow is the conversational FirstMate Captain and the only user
interface. It capability-checks a separately installed, unchanged stock FirstMate checkout, resolves a
project-scoped Captain Home, and starts exactly one visible normal FirstMate Ship as Kimiflow Main for the named
run. The start packet is an immutable snapshot of the user request plus an optional existing plan; it is not a
confirmed-contract claim. A start is green only when FirstMate's exact Main endpoint and Pi lifecycle are both
readable. Partial spawn output never becomes synthetic `started` or `recovered`.

Kimiflow Main has its own run-scoped FirstMate Home and standard Kimiflow Active Run inside its durable control
worktree. It owns current-code inspection, intake, research, product discussion, contract, plan, build,
verification and review. Main may start visible read-only `stage=research` Scouts before confirmation, then
implementation Ships and review Scouts after confirmation. Captain never spawns domain workers and Main never
writes supervised product bytes or existing run/plan artifacts in the original project. The exact stock FirstMate
status file named in Main's brief is the sole original-project control-state exception. Product writes and local
fast-forward delivery remain stock FirstMate child-Ship operations.

The adapter separates `FM_ROOT_OVERRIDE` from per-project/per-run `FM_HOME`, so projects do not share a fleet
lock. Before Main launch it rejects tracked `.kimiflow` paths under Kimiflow's existing local-artifact standard,
then establishes repository-local exclusion for untracked runtime state. If unchanged stock FirstMate cannot
name a non-main/master default branch, the adapter creates one owned reversible local `origin/HEAD` marker only
after proving the current branch locally (no remote) or from the authoritative `git ls-remote --symref` remote
HEAD. Existing, ambiguous, unreachable or mismatched markers/defaults are never overwritten or guessed.

An implementation Ship reports a committed, mechanically verified `paused` review-ready checkpoint. Main
dispatches visible review Scouts against that exact branch/commit, verifies their candidates, steers the same
Ship, and integrates through stock `fm-merge-local.sh` only. A real product decision becomes one bounded
`needs-decision` artifact pointer in Main status. Captain reads it, discusses it with the user, and sends the
answer back through stock `fm-send`; Main and children never open another user conversation.

FirstMate alone owns briefs, worktrees, Pi/Herdr endpoints, status/replies, wake, recovery, delivery and cleanup.
Kimiflow stores no parallel lifecycle truth and never implements Calm. Captain passes its resolved display level
to Main and Main passes it to every child. `quiet` loads stock FirstMate Calm in each new visible process; an
already running process is never claimed to have changed presentation retroactively. Missing or incompatible
FirstMate capability fails closed. Role action authority is tool-mechanical; Pi's shell is not OS-sandboxed, so
bounded briefs plus Kimiflow integration/final Git and evidence gates enforce the product-write boundary.
FirstMate leaves ordinary Pi/Kimiflow usable without a visible crew claim. Old custom Kimiflow Pi/Herdr bridge
receipts are diagnostic-only and not resumable.

`fm-session-start.sh` may drain durable wake records while acquiring either role's FirstMate Home. The adapter
returns those rows as `startupWakes`; the owning Captain or Main resolves named tasks through stock status before
new dispatch and does not drain already consumed rows again.

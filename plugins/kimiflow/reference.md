...
For a normal write run, derive the slug and call `workspace-preflight.sh route --run .kimiflow/<slug> --write`. A clean primary with no other active owner returns `direct` and writes no broker file. A dirty or busy primary first journals a random identity, branch, path and base commit, then creates exactly one locked `codex/<slug>*` worktree and publishes the existing registry plus Git-administration receipt. A matching interrupted allocation is recovered; mismatched path/branch/base/lock identity fails closed. The next run queues FIFO and retries autonomously after retirement. The cap counts only the single Kimiflow-owned tree: manual and Codex worktrees are inventoried/collision-checked but never claimed, unlocked, moved, integrated or deleted. A run already active on primary stays there even when dirty. Ordinary busy/dirty routing is not a workspace question; only the fate of genuinely ambiguous foreign/manual bytes may use the one durable batched `workspace` wait. Never force-remove, reset, clean, stash, delete branches, or stage/commit foreign paths.

Run `working-tree-gate.sh` in the selected routed root before product writes.

Phase 3 binds exact normalized affected paths and inferred/shared contracts to the SHA-256 of the final `PLAN.md` through `declare`. Exact/prefix path overlap and shared critical contracts serialize; same-boundary ambiguity reports `semantic-review` and safely serializes unless bounded semantic evidence proves separation. The comparison includes primary dirty paths, commits since allocation, other broker declarations, and dirty/committed paths from foreign/manual/Codex trees. Phase 5 recomputes the digest and requires `write-gate` OPEN. A stale plan returns to declaration; a collision waits/retries without user approval. Broker state is bounded local JSON, atomically written without symlink following; it stores only identities, paths/contracts, commit IDs, check argv/exit status, journals and archive receipts.

After the task's named-path commit, `integrate` runs from primary with one or more `--check-json '["program","arg"]'` entries; it never invokes a shell. It requires the current byte-bound PLAN declaration, exact registry/receipt ownership, a free/clean primary, a clean task tree apart from bounded `.kimiflow/` state, and `git merge-tree --write-tree --quiet`. If primary diverged, Kimiflow reclassifies newly committed paths and, only while disjoint, journals refs and makes one merge commit of the pinned primary into the owned task branch—never rebase/amend/force. All fallible project checks run on the fully reconciled task before delivery; primary identity is then revalidated and fast-forwards only. Post-delivery receipts are limited to mechanical commit-ancestry/index/worktree integrity, so a project-check failure can never occur after main was advanced. Broker routing, Active-Run start and integration share one primary lease; a losing start re-routes autonomously before any code write. Conflicts, semantic overlap, check failure or ref drift leave a stable recoverable state. Crash recovery accepts only the journaled reconciliation/fast-forward transition. After integration, finish the Active Run in the task root; `retire` then requires terminal run state, green pre/delivery receipts and integrated-head ancestry in primary before unlocking and calling the guarded archive path. Its journal can finish the exact identity-bound metadata archive after a crash between checkout and Git-administration retirement. Only ignored `.kimiflow/` state is tolerated; all bytes and the matching Git metadata are archived and the branch survives. The legacy explicit `register`/`remove` interface remains strict and compatible. Codex-managed paths below `$CODEX_HOME/worktrees` remain app-owned. Sources: https://git-scm.com/docs/git-worktree · https://git-scm.com/docs/git-status · https://git-scm.com/docs/git-merge-tree · https://docs.github.com/en/desktop/making-changes-in-a-branch/managing-worktrees-in-github-desktop
...
- **Evidence cascade / review lease:** the first review round always saturates every scheduled axis. After a
  verified material repair, the optional `review-cascade-record|resolve` route may lease unchanged axes instead
  of semantically rerunning them. The exact key includes model, opaque execution variant, role, task class,
  runtime, policy, and prompt/gate fingerprints: a new Frontier model, prompt profile, or gate therefore starts
  on full review and calibrates independently. Holdout + Shadow + five clean Canary A/B samples must show passed
  quality and verification, zero missed material findings/retries/audit findings, no additional rounds, at most
  two cascade rounds, and fewer measured tokens. Only then may routine work use `selective`; a stable one-in-ten
  audit forces full review. The latest bad sample, critical/unknown risk, repeated failure, regression, broad or
  boundary-changing deltas, stale route bytes, or a path-sensitive omitted axis revokes the lease.
  `review-convergence-gate.sh delta` binds the previous schema-2 saturation, exact repair and PLAN digests,
  current per-path bytes/modes, changed paths, axis partition, and route receipt into `review-deltas/r<N>.json`;
  schema-2 saturation binds that receipt. This removes redundant semantic rereads only. Tests, falsifiers,
  candidate disposition, evidence, aggregate resolution, and the final whole-intent check remain mandatory.
...
- **Code saturation is mechanically bounded to emitted evidence and stable bytes.** For a new contracted code round, run `review-convergence-gate.sh basis --run <run> --base <validated-base> --details` and copy its exact base/target/snapshot plus per-path `review_files` into schema-2 `review-saturation/r<N>.json`, together with exact `round`, current `plan_sha256`, `scheduled_axes`, actual `axes`, each candidate file's SHA-256, `carried_classes`, and one disposition per emitted BLOCKER/HIGH candidate. Full rounds set `delta_receipt=null` and actual axes equal scheduled axes. The snapshot covers HEAD plus every STATE-declared affected path including file type/existence, so any later source mutation makes that receipt stale. Candidate identity is `cand_` plus SHA-256 of `axis + NUL + exact candidate line`; a disposition carries `promoted|refuted`, stable class, the exact meaningful typed method, and reproduced/not-reproduced evidence. `command:true`, `:`, standalone output commands, and vague verifier prose are invalid. Multiple axes may map the same underlying class only when verify/evidence agree; the aggregate carries that class once. A carried class must match the immediately preceding aggregate's verify/evidence identity exactly. A leased partial repair round first requires `review-convergence-gate.sh delta` to validate the exact previous schema-2 saturation and repair digests, unchanged PLAN, per-path repair delta, path-required axes, and exact active route receipt; the resulting `review-deltas/r<N>.json@<sha256>` becomes `delta_receipt`. Run `review-convergence-gate.sh saturation --run <run> --round N --axes <actual-csv>` before the resolver; contracted code resolution repeats it with the same actual axes. Missing axes, changed source/candidate/plan/route bytes, unsafe or over-broad deltas, undisposed material candidates, forged/drifted carry-forward, mixed outcomes, or aggregate mismatch close. Schema-1 receipts remain resumable for older runs. This proves axis completion and disposition of every emitted material candidate; it cannot prove a model noticed an un-emitted latent defect.
- **Repair batching replaces finding-by-finding patching.** When the aggregate has open BLOCKER/HIGH, write `review-repairs/r<N>.json` with exact round, current plan/findings SHA-256 and `groups`. Each bounded group has an ID, one or more stable classes, one concrete shared `root_cause`, `depends_on`, one coherent `repair`, and one or more meaningful typed `{kind,method}` checks. At least one check per group exactly reuses a covered finding's authoritative verify method; change that method only through a newly verified finding/evidence receipt. Across groups every open class appears exactly once and dependencies are acyclic. `review-convergence-gate.sh repair --run <run> --round N` must return `OPEN/repair-ready` before Phase 5 changes code; no open classes returns `OPEN/not-required`. This artifact uses already verified findings and adds no model seat.
...
- **Gate count (mechanical, current round only) — delegated to the tested resolver.** The orchestrator runs `${CLAUDE_PLUGIN_ROOT:-$CLAUDE_SKILL_DIR}/hooks/resolve-review-gate.sh .kimiflow/<slug>/findings --round <N> --expect <lensCSV> --gate <plan|code> --epoch-start <S> --cap <C>`, adding `--finding-contract 1` when STATE declares Convergence Contract 1 and, for contracted code, `--review-axes <csv>`. `S` is the current strategy epoch's first global round and absolute `C=S+B-1` (`B=2` for small/quick, `B=3` for large/audit/release-critical). `PLAN.md` is the canonical strategy basis for both gates. Before each gate's round 1, `RECOVERY.md` gets exactly one `<!-- kimiflow:strategy gate=<plan|code> epoch-start=1 fingerprint=<sha256(PLAN.md)> -->`; explicit gate-aware calls require and recompute it. Legacy/noncontracted calls may omit `--epoch-start`; a contracted `code-verified` call requires explicit `--gate code`, `--epoch-start`, `--cap`, `--finding-contract 1`, and `--review-axes` so it cannot fall back around saturation/trajectory validation. The script is the **single source of truth**: it validates completeness, grammar, evidence safety/digests, negative resolution, and strategy-epoch liveness; contracted code calls also repeat trajectory and saturation validation. It then echoes `VERDICT⇥count⇥reason_code⇥detail`. Only `OPEN/clean` advances. `incomplete|malformed|unproven-resolution` repairs transport/evidence; `open-findings` permits the next targeted repair; `root-class-repeated|oscillation|reappeared|cap-reached` starts autonomous strategy recovery—never a continue prompt. It is language-agnostic and unit-tested by `hooks/test-resolve-review-gate.sh`; it never reads `REVIEW.md` or emits `OPEN` for recovery.
